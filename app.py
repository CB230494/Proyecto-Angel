...
with g3:
    altura_si = st.slider("Altura cajas SÍ (px)", 70, 110, 78, 2)  # NUEVO: altura global SÍ
...
with c7:
    comp_branch = st.slider("Compacidad rama SÍ (0.20–1.00)", 0.20, 1.00, 0.45, 0.05)  # NUEVO: toda la rama
with c8:
    comp_early  = st.slider("Compacidad extra SÍ 1–5", 0.20, 1.00, 0.35, 0.05)         # NUEVO: primeros 5
with c9:
    vert_margin = st.slider("Margen vertical flecha (px)", 24, 70, 30, 2)              # NUEVO: margen mínimo
...
def label_near_segment_outward(d, p1, p2, text, page_center_x, offset=36):
    mx, my = (p1[0]+p2[0])/2, (p1[1]+p2[1])/2
    dx, dy = p2[0]-p1[0], p2[1]-p1[1]; L=max(1.0, math.hypot(dx,dy))
    nx, ny = -dy/L, dx/L
    cand1=(mx+nx*offset, my+ny*offset); cand2=(mx-nx*offset, my-ny*offset)
...
def render_png() -> bytes:
    ...
    widths = int(ancho_si)
    # Alturas por nodo (SÍ5 independiente)
    heights=[int(altura_si), int(altura_si), int(altura_si), int(altura_si), int(altura_si5),
             int(altura_si), int(altura_si), int(altura_si)]

    max_center_y = r_fin[1]-40
    min_center_y = (r_dec[1]+r_dec[3])//2 + 10

    # requisitos mínimos con margen ajustable
    req=[(heights[i]/2 + heights[i+1]/2 + vert_margin) for i in range(n-1)]

    # multiplicadores de compacidad
    ...
    # base_step máximo que cabe (binaria) y limitado por tu slider
    ...
    poly_arrow(d,[start,mid1,mid2,end]); d.text((min(W-50,rail_x-10),(start[1]+mid2[1])//2),
        "Retroalimentación", font=FONT_SMALL, fill=BLUE, anchor="rm")

    out=io.BytesIO(); Image.open(io.BytesIO(img.tobytes()) if False else None)  # no-op para mypy
